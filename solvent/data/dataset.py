...
class EnergyForceDataset:
    ...
    def __len__(self) -> int:
        if not self._is_loaded:
            raise utils.DataNotLoadedException('must load dataset before accessing dataset length.')
    ...
    def _load_structure(self, idx: int) -> Data:
        ...
        if self._units == 'EV':
            energies = utils.hartree_to_ev(energies)
            forces = utils.hartree_to_ev(forces)
        elif self._units == 'KCAL':
            energies = utils.hartree_to_kcal(energies)
            forces = utils.hartree_to_kcal(forces)
        return Data(
            x=one_hot_vecs,
            pos=shifted_coords,
            energies=energies,
