...
for i, log_file in enumerate(LOG_FILES):
    ...
    axs[i].plot(epochs, train_mae, label='Train MAE', linestyle=':')
    axs[i + 1].plot(epochs, train_mse, label='Train MSE', linestyle=':')
    axs[i].plot(epochs, test_mae, label='Test MAE', linestyle='-')
    axs[i + 1].plot(epochs, test_mse, label='Test MSE', linestyle='-')
    axs[i].legend()
    axs[i + 1].legend()
plt.show()
